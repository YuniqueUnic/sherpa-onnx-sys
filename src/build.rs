...
fn download_prebuilt_libs(
    ...
    let unpack_result: Result<(), DynError> = (|| {
        if archive_name.ends_with(".xcframework.zip") {
            ...
            // Keep each archive self-contained under its versioned extraction
            // directory. Android releases contain a top-level `jniLibs/`
            // directory; unpacking into the cache root makes the resolver
            // look in `<stem>/jniLibs` while the files actually land in the
            // sibling cache root and breaks every clean Android build.
